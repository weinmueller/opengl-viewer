...
// Utility functions for calculating screen-space size and selecting LOD levels
namespace LODSelector {

// Default LOD thresholds (in screen pixels) - higher values = keep detail longer
constexpr float LOD0_THRESHOLD = 400.0f;  // Highest detail
constexpr float LOD1_THRESHOLD = 200.0f;
constexpr float LOD2_THRESHOLD = 100.0f;
constexpr float LOD3_THRESHOLD = 50.0f;
constexpr float LOD4_THRESHOLD = 25.0f;
constexpr float LOD5_THRESHOLD = 0.0f;    // Lowest detail

// Default triangle ratios for each LOD level - gentler reduction
constexpr float LOD0_RATIO = 1.0f;        // 100%
constexpr float LOD1_RATIO = 0.7f;        // 70%
constexpr float LOD2_RATIO = 0.5f;        // 50%
constexpr float LOD3_RATIO = 0.35f;       // 35%
constexpr float LOD4_RATIO = 0.25f;       // 25%
constexpr float LOD5_RATIO = 0.15f;       // 15%

// Hysteresis buffer to prevent LOD popping (10% of threshold)
constexpr float HYSTERESIS = 0.1f;
...
inline float calculateScreenSize(
...
inline int selectLOD(float screenSize, int currentLOD, int lodCount) {
